    ...
    def analyze_trends(self, data):
        ...
        for precinct_data in data:
            ...
            if len(years) == 2:
                ...
                # Safety level
                if latest_violent < 100:
                    safety = "relatively safe"
                elif latest_violent < 300:
                    safety = "moderately safe but requires caution"
                else:
                    safety = "an area requiring increased vigilance"
                
                analysis += f"<li><strong>Analysis:</strong> Based on recent crime statistics, Precinct {precinct} is {safety}.</li>"
                analysis += f"<li><strong>Total crime changed by {total_change:.1f}%</strong> from {first_year} to {latest_year}.</li>"
                analysis += "<li><strong>RECOMMENDATION:</strong> The area is generally safe for regular activities, but take normal urban precautions.</li>"
